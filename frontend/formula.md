...
**Identity:** when `B = 1` (Battery Size = 100%), `UF` always equals exactly `1` (100%),
regardless of `D` or the multiplier — the `0.5 × (N + N)` term collapses to `N`, so `D + N = 1`.
...
**Off-Grid / Hybrid** — Grid-Tied-equivalent hardware cost (one fixed number, the *average* of
the Grid-Tied range, not a min/max pair) **plus** the actual cost of the battery the system
needs — only the battery term varies between min and max:

```
GridTiedAverage = (Capacity × GRID_TIED.perKwMin + Capacity × GRID_TIED.perKwMax) / 2
BatteryKwh      = see §2, using this type's Daytime Use / Battery Size

Investment.min = GridTiedAverage + BatteryKwh × BatteryRate.perKwhMin
Investment.max = GridTiedAverage + BatteryKwh × BatteryRate.perKwhMax
```

| Constant | Off-Grid | Hybrid |
|---|---|---|
| `perKwMin` / `perKwMax` (Grid-Tied-equivalent, shared) | ₱35,000 / ₱50,000 | ₱35,000 / ₱50,000 |
| `BatteryRate.perKwhMin` / `perKwhMax` | ₱12,000 / ₱25,000 | ₱15,000 / ₱30,000 |
...
real time as you drag a slider, and converges with §7a once Apply Configuration commits it.
